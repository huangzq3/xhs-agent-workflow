...
def fit_aspect(img: Image.Image, ratio: str) -> Image.Image:
    ...
    try:
        ...
        raise ValueError(f"bad aspect ratio {ratio!r}; expected W:H") from exc
    if w_r <= 0 or h_r <= 0:
...
def parse_size(value: str) -> tuple[int, int]:
    try:
        ...
        raise ValueError(f"bad canvas size {value!r}; expected WIDTHxHEIGHT") from exc
...
def main() -> int:
    ...
    ap.add_argument("--canvas-size", default="900x1200",
                    help="resize after crop, e.g. 900x1200; empty keeps cropped size")
    ap.add_argument("--padding", type=int, default=48)
    args = ap.parse_args()

    src = ImageOps.exif_transpose(Image.open(args.input)).convert("RGBA")
    src = fit_aspect(src, args.fit)
    if args.canvas_size:
        src = src.resize(parse_size(args.canvas_size), Image.Resampling.LANCZOS)
    W, H = src.size
