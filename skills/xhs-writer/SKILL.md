...
1. 内部要求明确的工作区、本轮任务、已确认的账号运营策略、账号定位、选题报告和已选选题；定位追踪遵守 [定位生长闭环](../xhs-workflow/references/positioning-loop.md)。
...
1. 读取选题核心、受众需求、证据、风险、账号定位边界、创作者方向及其账号落地，以及运营策略中的内容目标组合。
2. 为本篇明确一个业务目标：“吸引新受众”“建立信任”或“强化账号标签”；从已确认选题原样复制 `positioning_trace`，包括 Persona revision、内容支柱、受众任务、交付价值、证据职责、主记忆资产、待验证假设和探针角色。试运营内容在内部记录序号，不要求内容负责人理解内部枚举。
...
10. 写入 `content` JSON，记录修订号、`positioning_trace`、`change_summary` 和可追溯的 `authorship`；新稿的 `article_audit_ref` 必须为 null，由核心工作流在独立审计完成后绑定。正文无法履行既定定位任务时退回选题，不得只改追踪字段以通过校验。
11. 运行核心契约与素材校验，生成中文 HTML 预览；已由 Agent 原生生图能力生成的图片还要完成视觉核对，并用业务语言展示相对上一版的修改内容。能力缺失时只交付 prompt、布局规格、未执行任务或人工素材任务，不在本地生成卡片图。
...
内容必须履行选定内容支柱的价值与证据职责，但商业连接不等于每篇都需要产品提及或销售召唤。

`positioning_trace` 与已确认选题不一致时，G3 必须阻断。内容负责人临时改变受众、价值、证据职责、记忆资产或定位假设，属于选题修订或定位修订，而不是普通改稿。
...
- 不调用 Pillow 或本地栅格脚本生成文字卡、叠字、拼图、裁剪结果；图片内容只由当前 AI Agent 已展示的原生生图能力生成或编辑。
