...
| content | xhs-writer | article-audit、publish、review | 文案、卡片/分镜、事实与权利台账、作者身份与审计引用 |
...
`article_audit` 使用独立 payload 契约版本 `1.0.0`，与包的工作流 Schema 版本分开演进。通用文件审计可使用 `schema_version=article-audit/1.0.0` 和空的账号/任务标识；xhs 接入使用核心 `2.2.0` 信封并要求账号与任务一致。它不保存人工批准，只保存审计事实；人工取舍始终记录在 content 的内容定稿确认中。定制品牌、语气或合规规则只能通过显式 `custom_profile_refs` 叠加，不得写入通用核心规则。
