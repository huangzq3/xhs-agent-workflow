...
| 发布 | 目标账号、最终预览、发布时间和规则例外 | 是否授权一次发布尝试 |
| 数据复盘 | 流量、信任、长尾表现和其他可能解释 | 是否接受复盘与下一步建议 |
...
| xhs-publish | 发布规则检查、最终预览、一次发布尝试与结果核对 |
| xhs-content-review | 短期、信任与长尾数据复盘 |
...
内部机器事实源仍为 JSON，Schema 版本仍是 2.2.0。V2.2.1 只调整人工交互和报告层，不改变已有机器字段或状态机。
...
测试覆盖机器契约、账号策略与定位分层、人工确认失效、发布防重、内容库存、长尾复盘、HTML 安全转义、中文术语映射、人工审计 HTML、原生生图交接、素材权利和去水印阻断。
